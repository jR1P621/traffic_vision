# Lidar class by Arash Javanmard
# https://github.com/ArashJavan/veloparser

# Reads/Decodes raw LiDAR TCP stream and extracts data.

import datetime
import numpy as np

from tools.lidar.gps import GprmcMessage, utc_to_weekseconds


def read_uint32(data, idx):
    return data[idx] + data[idx + 1] * 256 + data[idx + 2] * 256 * 256 + data[
        idx + 3] * 256 * 256 * 256
...
class VelodyneVLP16(Lidar):
    ...
    def calc_timing_offsets(self):
        timing_offsets = [[0.0 for x in range(12)]
                          for y in range(32)]  # Init matrix
    ...
    def process_position_frame(self, data, frame_idx):
        ...
        gprmc = data[206:]
        gprmc = gprmc.split()[
            0]  # filter out gprmc message, remaining are zeros
        ...
        time = gprmc[1]
        date = gprmc[9]
        gps_msg.datetime = datetime.datetime.strptime(date + time,
                                                      '%d%m%y%H%M%S')
    ...
    def calc_precise_azimuth_2(self, azimuth):
        ...
        for n in range(12):  # n=0..11
            ...
            for k in range(32):
                ...
                if k < 16:
                    # Interpolate
                    precise_azimuth = azimuth[n] + (azimuth_gap * 2.304 *
                                                    k) / 55.296
                else:
                    # interpolate
                    precise_azimuth = azimuth[n] + (azimuth_gap * 2.304 * (
